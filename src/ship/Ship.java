...
public class Ship implements Runnable {
	...
	public void run() {
		try {
			...
			logger.error("Корабль не заплыл в порт и уничтожен в море", e);
		}
	}
	...
	private void inPort() throws PortException, InterruptedException {
		...
		try {
			...
			if (isLockedBerth) {
				...
				logger.debug("Корабль " + name + " пришвартовался к причалу "
						+ berth.getId());
				ShipAction action = getNextAction();
				executeAction(action, berth);
			} else {
				logger.debug("Кораблю " + name
	...
	private boolean loadToPort(Berth berth) throws InterruptedException {
		...
		int containersNumberToMove = containersCountToLoadFromShip();
		boolean result = false;

		logger.debug("Корабль " + name + " хочет загрузить "
		...
		if (!result) {
			...
			logger.debug("Корабль " + name + " выгрузил "
	...
	private boolean loadFromPort(Berth berth) throws InterruptedException {
		...
		int containersNumberToMove = containersCountToLoadToShip();
		...
		if (result) {
			logger.debug("Корабль " + name + " загрузил "
					+ containersNumberToMove + " контейнеров из порта.");
		} else {
			logger.debug(name + ": недостаточно контейнеров на складе порта"
					+ " для выгрузки " + containersNumberToMove
	...
	// added
	private int containersCountToLoadFromShip() {
		if (shipWarehouse.getRealSize() == 1)
			{return 1;}
		Random random = new Random();
		int amount = random.nextInt(shipWarehouse.getRealSize() - 1) + 1;
		return amount;
	}

	// added
	private int containersCountToLoadToShip() {
		if (shipWarehouse.getFreeSize() == 1)
			{return 1;}
		Random random = new Random();
		return random.nextInt(shipWarehouse.getFreeSize() / 2) + 1;
	}
